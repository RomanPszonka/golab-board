...
| C-2b | High | Persistent poison-pill: empty/invalid/**compressed** `TR`/`SQ` mark → nil-deref in `GenerateFullFrame` (sibling of C-2; a spec-valid `TR[aa:cc]` from any Go SGF client also triggers it) | PoC `b2` | Yes ⚑ |
...
- **Delivery (unauthenticated):** upload an SGF whose current node has an empty or malformed mark — `(;SZ[19]SQ[])`, `(;TR[])`, `(;SQ[!])`, `(;TR[dd][])`. `FromSGF` accepts it and `UploadSGF` commits the state **before** frame generation; `ToSGFIX` writes the bad mark back, so it **persists** and re-bricks the room on every join/reload. Same escalation as C-2: whole-server crash if the room's OGS plugin is active (`GenerateFullFrame` runs in the OGS goroutine).
- **Structural scope (SGF-aware audit + `b2` controls).** The trigger is broader than malformed input: `coord.FromLetters` wants exactly two letters, but SGF point lists may use a **compressed rectangle** `TR[aa:cc]` (a spec-valid range that KGS, OGS, Sabaki, gomill all emit). `FromLetters("aa:cc")` is length 5 → nil → same crash. **So an honest SGF exported from another Go/Go-web client, with a marked region, bricks the board — no attacker needed** (an interop bug, not just an injection). Conversely, auditing *every* mark/annotation property shows the sink is bounded to exactly **`{TR, SQ, LB}`**: `CR`, `MA`, `SL`, `AR`, `LN`, `DD` are stored but never rendered by `generateMarks`, so they do **not** crash (verified as `b2`'s "safe controls", and why the pipeline fuzzer converges on TR/SQ/LB in seconds).
- **Tooling:** re-found in <5 s by the new `FuzzSGFPipeline` (see §6 — *Fuzzing*), which exercises `FromSGF → GenerateFullFrame → serialize → reload`; the old parser-only fuzzers ran millions of execs clean because they never called `GenerateFullFrame`.
- **Fix:** nil-check in `CoordSet.Add` (and skip nil coords in `generateMarks`); validate `TR`/`SQ`/`LB` values in `FromSGF`, and **expand compressed `aa:cc` ranges** before `FromLetters`, so malformed *and* legitimate-compressed marks are handled. (The single `CoordSet.Add` guard closes the whole `FromLetters`→`Add` family.)
...
### Fuzzing & test tooling (updated this pass)

The repo ships three fuzz targets — `FuzzSGFParser`, `FuzzNGFParser`, `FuzzGIBParser` (`parser_test.go`) and `FuzzFromSGF` (`state_test.go`). **All four stop at the parser / `FromSGF`.** But every SGF poison-pill in this report (C-2 `LB[z]`, C-2b `TR[]`/`SQ[aa:cc]`, M-5 the `\`-escaping) *parses cleanly* and only crashes later — in `GenerateFullFrame` (`generateMarks`) or after a serialize→reload round-trip. That is precisely why those fuzzers ran millions of execs green while the bugs sat live: **they never render or round-trip the parsed tree.**

- **Added `FuzzSGFPipeline`** (`pkg/state/state_test.go`) — drives the *whole* untrusted-SGF path a real upload takes: `FromSGF → GenerateFullFrame(Full/CurrentOnly) → ToSGF/ToSGFIX → FromSGF → GenerateFullFrame`. It rediscovers the mark-nil-deref sink in **under 5 seconds** (`go test -run x -fuzz FuzzSGFPipeline ./pkg/state/`). The committed seed corpus is deliberately benign so the normal `go test` stays green; a fuzz run writes the crasher to `testdata/fuzz/`. This is the target to keep running (and to seed with the fixed poisons as regression cases once patched).
- **Added `TestL10_MemoryLoaderConcurrentMap`** (`security/poc/race/`, `//go:build race`) — reproduces the L-10 unsynchronized-map crash under `go test -race` (fires a `DATA RACE` on `SaveRoom`/`DeleteRoom` vs `LoadAllRooms`). It is race-tagged so it never fatals a plain `go test ./...` run.
- The existing `security/poc/race/` torn-read tests (DR-2/DR-3) and the parser fuzzers remain valid for their scope; no change needed there.
...
- **L-10 — `MemoryLoader` is not mutex-protected** (`pkg/loader/memoryloader.go:17`): `rooms`/`twitch` maps and the `messages` slice are read/written from every room goroutine + the persist loop with **no lock** → `fatal error: concurrent map …` (whole-server, unrecoverable). Reproduced under the race detector (`go test -race -run TestL10 ./security/poc/race/` → `DATA RACE` on `SaveRoom`/`DeleteRoom` vs `LoadAllRooms`). **Low only because it is memory-mode-only** — the assessed production posture uses a sqlite/postgres loader (concurrency-safe via `database/sql`). *If the in-memory loader is ever the configured backend, this is a Critical-class unauthenticated crash* (two clients creating rooms concurrently). Fix: guard the loader with a `sync.Mutex`, or don't run memory mode in production.
...
- **L-13 — navigation reads `Down[PreferredChild]` with no bounds check** (`pkg/state/nav.go` `right()`; `SetPrefs` at `util.go:58` copies persisted prefs in without validating them against `len(Down)`). Confirmed to panic — `index out of range [99] with length 1` — whenever a node's `PreferredChild` exceeds its child count. Latent only: `FromSGF` resets all prefs to 0 and normal navigation keeps them in range, so no client-reachable path to persist an out-of-range pref was found (the earlier "stale `PreferredChild`" plausible, now characterized). Recovered per-connection even if reached. Fix: clamp in `right()`/`down()` and validate in `SetPrefs`.
- **Plausible, not confirmed (no PoC):** other OGS frame-reader assertions beyond `gamedata` (`ogs.go:264-318`, same class as H-9); `tree.Copy` recursion on a *deep linear* tree (subsumed by C-3/C-4, which overflow on parse/serialize first); a persisted deep tree re-overflowing `parseBranch` in `Hub.Load` (narrow — the upload path overflows before it can be saved, so only the graft-built path could persist one). Each is a candidate for a follow-up pass but none is a demonstrated exploit here.

**Adversarial command/scoring/format sweep (this pass — surfaces confirmed clean or bounded).** To answer "anything left," every remaining client-reachable surface was walked; these held up:
- **All 25 WebSocket command types** were audited. The mark-writing commands (`triangle`/`square`/`letter`/`number`/`label`, `commands.go:146-240`) each **bounds-check the coord and emit a valid 2-letter point via `ToLetters()`**, so — unlike an uploaded SGF — they **cannot** deliver the C-2/C-2b mark poison. C-2b's delivery is therefore **upload-only** (`upload_sgf`/`request_sgf`), not the command path.
- **The scoring path is robust.** `score`/`markdead` are unauthenticated on an open room and reach `Board.Score` / `FindGroup` / `FindArea` / `detectAtariDame`, but the flood-fills use an **explicit stack (no recursion → no stack overflow)** and the atari-dame loop is monotonic (fills only, so it terminates). Verified no crash on empty and adversarial boards.
- **`goto_grid`/`goto_coord`** feed a client integer to `gotoIndex → setPreferred`, which **nil-checks the node** (`util.go:29`) and returns an error for an unknown index — no panic.
- **GIB & NGF are auto-detected inside `upload_sgf`.** `state.FromSGF → parser.Parse` sniffs clean-SGF → GIB (`\HS…`) → NGF → dirty-SGF (`parser.go:109`), so those parsers *are* attacker-reachable. Their parse panics are the recovered M-10 class (request-path), and any GIB/NGF field they emit (`PB`/`PW`/`GN` player names/titles) rides the **same `]`-only escaping as M-5** if it ends in `\`. No *new* crash class, but a wider delivery surface for M-5 and the parser panics than "SGF only."
...
- **`remove_mark value[:2]` and GIB `alphabet[x]` panics** — real panics, but recovered per-connection (M-10 class). GIB/NGF *are* reachable (auto-detected inside `upload_sgf`), but the parser's **output** is clean SGF coordinates that are never re-parsed as GIB, so the coordinate panic itself is not a poison-pill. (Caveat: GIB/NGF-sourced `PB`/`PW`/`GN` text still rides the M-5 `\`-escaping like any other text field.)
...
Run with no arguments for the full list of harness commands (the newest additions: `copybomb` C-8, `sizepoison` M-11, `sgfquad` M-12 — all local-only, no target needed). Data races use `go test -race ./security/poc/race/` (includes L-10, the MemoryLoader map race); the OGS gamedata crash (H-9) uses `go test -run TestOGSGamedataCrash_1c ./pkg/room/plugin/`. **Fuzzing:** `go test -run x -fuzz FuzzSGFPipeline ./pkg/state/` drives the full parse→render→reload path and re-finds the mark poison-pill in seconds (the parser-only fuzzers miss it). **Browser PoCs** (need Node + Playwright + Chromium, and the server running): `node security/poc/browser/xss_label.js` (stored XSS) and `node security/poc/browser/clickjacking.js`. Several PoCs crash or exhaust the target — **authorised local testing only**; see [`security/poc/README.md`](security/poc/README.md).
